 ...
 */  
  ...
  $L['createuser'] = 'Создать пользователя';
 ...
 /*
   * Help
   */
  $L['userman_help_main'] = '<b>TGA</b> - Временный доступ к группам ( Temporary groups access )<br /><br /> Если поле <b>TGA</b> пустое, то временный доступ пользователю не установлен, и в БД нет записи для пользователя. Если в поле есть иконка <img class="icon" src="plugins/userman/inc/access.png" alt="запись активна" title="запись активна" />, то пользователь есть в базе для временного доступа к группам и запись активна.
      Если в поле есть иконка <img class="icon" src="plugins/userman/inc/access_off.png" alt="запись отключена" title="запись отключена" />, то пользователь есть в базе для временного доступа к группам, но запись не активна - временный доступ пользователю отключён.<br /><br /> Для редактирования доступа пользователя к дополнительным группам нажмите на иконку в поле <b>TGA</b>.<br /><br />
      Для создания пользователю временного доступа к группам, зайдите в профиль пользователя кликнув по имени пользователя в списке, и там воспользуйтесь кнопкой - "Изменить временный доступ к группам".<br /><br /> Для редактирования профиля пользователя нажмите на его имя в списке имён.<br /><br /> Для поиска пользователя по E-mail введите E-mail в поле для поиска по имени.';
  $L['userman_help_access'] = 'Временный доступ к группам удобен, когда, например, у вас на сайте орагнизован платный доступ к неторорым разделам и вы открываете доступ пользователям на время, после оплаты.<br />
      ...
      то запись из базы не удаляется, а просто отключается переводом <b><i>Профиль активен</i></b> в состояние <b><i>НЕТ</i></b>.<br /><br /><b>Внимание !</b><br /> Если временный досту пользователя к группам активен - <img class="icon" src="plugins/userman/inc/access.png" alt="запись активна" title="запись активна" />, редактирование доступа к группам нужно изменять с этой страницы
      - страницы временного доступа к группам. Вы не сможете изменить доступ пользователя к группам из других страниц.<br />Если профиль не активен - <img class="icon" src="plugins/userman/inc/access_off.png" alt="запись отключена" title="запись отключена" /> или отсутствует в базе временного доступа, вы можете изменять права доступа к группам с любой страницы сайта, позволяющей редактровать доступ к группам.';
